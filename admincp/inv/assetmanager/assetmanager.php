...
if ($sBase1 != "")
	$sBaseRoot1 = ereg_replace($sBaseVirtual1, "", $sBase1);
if ($sBase2 != "")
	$sBaseRoot2 = ereg_replace($sBaseVirtual2, "", $sBase2);
if ($sBase3 != "")
	$sBaseRoot3 = ereg_replace($sBaseVirtual3, "", $sBase3);
...
$MaxFileSize = 3000000;
$AllowedTypes = "gif|jpg|pdf|zip|doc|xls|rar|bmp|wma|mp3|wav|avi|dat|swf";
//$AllowedTypes = "*";
...
if (isset($_FILES["File1"])) {
	...
	} else if (move_uploaded_file($_FILES['File1']['tmp_name'],
			$currFolder . "/" . basename($_FILES['File1']['name']))) {
		$sMsg = "";
		$sUploadedFile = $_FILES['File1']['name'];
		@chmod($currFolder . "/" . basename($_FILES['File1']['name']), 0777);
	} else {
		$sMsg = "Upload failed.";
	}
} else {
...
if ($sBase0 != "") {
	if (strtolower($currFolder)
			!= ereg_replace(strtolower($sBase0), "", strtolower($currFolder))
			AND $bReadOnly0 == true)
		$bWriteFolderAdmin = true;
}
if ($sBase1 != "") {
	if (strtolower($currFolder)
			!= ereg_replace(strtolower($sBase1), "", strtolower($currFolder))
			AND $bReadOnly1 == true)
		$bWriteFolderAdmin = true;
}
if ($sBase2 != "") {
	if (strtolower($currFolder)
			!= ereg_replace(strtolower($sBase2), "", strtolower($currFolder))
			AND $bReadOnly2 == true)
		$bWriteFolderAdmin = true;
}
if ($sBase3 != "") {
	if (strtolower($currFolder)
			!= ereg_replace(strtolower($sBase3), "", strtolower($currFolder))
			AND $bReadOnly3 == true)
		$bWriteFolderAdmin = true;
}
$sFolderAdmin = "";
...
Function recursive($sPath, $sPath_base, $sName) {
	...
	if ($sPath == $sBase0 || $sPath == $sBase1 || $sPath == $sBase2
			|| $sPath == $sBase3) {
		if ($currFolder == $sPath)
...
function writeFileSelections() {
		...
		if ($sItem == "." || $sItem == "..") {
			...
			if (!$fIsDirectory) {
				...
				if ($ffilter == "flash") {
					...
					if ($sExt == "avi" || $sExt == "wmv" || $sExt == "mpg"
							|| $sExt == "mpeg" || $sExt == "wav"
							|| $sExt == "wma" || $sExt == "mid" || $sExt
									== "mp3")
						$bDisplay = true;
				...
				} else //all
 {
				...
				if ($bDisplay) {
					...
					$sCurrent_virtual = str_replace('../../../../', $linkS,
							$sCurrent);
					if ($sBaseRoot1 != "")
						$sCurrent_virtual = ereg_replace($sBaseRoot1, "",
								$sCurrent_virtual);
					if ($sBaseRoot2 != "")
						$sCurrent_virtual = ereg_replace($sBaseRoot2, "",
								$sCurrent_virtual);
					if ($sBaseRoot3 != "")
						$sCurrent_virtual = ereg_replace($sBaseRoot3, "",
								$sCurrent_virtual);
					...
					echo "<td valign=top style='cursor:pointer;' onclick=\"selectFile("
							. $nIndex . ")\" width=100% ><u id=\"idFile"
							. $nIndex . "\">" . $sItem . "</u></td>";
					echo "<input type=hidden name=inpFile" . $nIndex
							. " id=inpFile" . $nIndex . " value=\""
							. $sCurrent_virtual . "\">";
					echo "<td valign=top align=right nowrap>"
							. round(filesize($sCurrent) / 1024, 1)
							. " kb&nbsp;</td>";
					echo "<td valign=top nowrap onclick=\"deleteFile("
							. $nIndex
							. ")\"><u style='font-size:10px;cursor:pointer;color:crimson' "
							. $sFolderAdmin
							. "><script>document.write(getText('del'))</script></u></td>";
					echo "</tr>";
				}
			}
		}
	...
	echo "<input type=hidden name=inpUploadedFile id=inpUploadedFile value='"
			. $iSelected . "'>";
	echo "<input type=hidden name=inpNumOfFiles id=inpNumOfFiles value='"
			. $nIndex . "'>";
...
<script>
var bReturnAbsolute=<? if ($bReturnAbsolute) {
	echo "true";
} else {
	echo "false";
} ?>;
...
if ($ffilter == "flash")
	$sFlash = "selected";
$sHTMLFilter = $sHTMLFilter . "	<option name=optLang id=optLang value='' "
		. $sAll . "></option>";
$sHTMLFilter = $sHTMLFilter . "	<option name=optLang id=optLang value='media' "
		. $sMedia . "></option>";
$sHTMLFilter = $sHTMLFilter . "	<option name=optLang id=optLang value='image' "
		. $sImage . "></option>";
$sHTMLFilter = $sHTMLFilter . "	<option name=optLang id=optLang value='flash' "
		. $sFlash . "></option>";
