...
//var_dump($_SESSION['s_img_galary_id']);
// Settings
	$save_path =  $baseDir . $gUploadDir;				// The path were we will save the file (getcwd() may not be reliable and should be tested in your environment)
	$upload_name = "Filedata";
	$max_file_size_in_bytes = 2147483647;				// 2GB in bytes
	$extension_whitelist = array("jpg", "gif", "png", "doc");	// Allowed file extensions
...
$db=new DB_Sql;
$str_sql_del=" delete from $gTableName where recordText = '$file_name'";
 $db->query($str_sql_del);

$str_sql_ins=" insert into $gTableName ( recordText,img_galary_id) values ('$file_name','".$_SESSION['s_img_galary_id']."')";
 $db->query($str_sql_ins);
 
$str_sql_sel=" select max($gPosfield) as m from $gTableName  ";
if ($db->query_next($str_sql_sel))    $m = $db->f("m"); else $m=0;
$m += 1;

$str_sql_upd="update $gTableName set $gPosfield= $m where $gPosfield is null";
$db->query($str_sql_upd);

if (count($arr_notice_messages))
    	{
            HandleError("sql error" . join("\n", $arr_notice_messages));
            exit(0);
        }
