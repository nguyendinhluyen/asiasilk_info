<?php
if($_SESSION['TIMEOUTN'] + $timeout < time())
{
	...
	if(check_id_news($id))//fill data on textbox : check_valid($id)
	{
		...
		$title = $panel;
		$sql = "Select news_name,keywords,description,news_image,news_catalogue,news_shortcontent,news_content from news where news_id = '".$id."' and language='".$_SESSION['lag']."'";
		$mysql -> setQuery($sql);
		...
		$txtdetail = output($row['news_content']);
		$keywords = $row['keywords'];
		$description = $row['description'];
		$news_shortcontent = $row['news_shortcontent'];
	...
	if(isset($_POST['btnadd']))
	{
		...
		$txtdetail = output($_POST['txtdetail']);
		$news_shortcontent = $_POST['news_shortcontent'];
		...
		//if(!isset($txtdetail{19})) $error.='<li>Nội dung tin tức quá ngắn</li>';
		...
		if(empty($error))
		{
			if($update==1)//update
			{
				$sql = "update news set news_key ='".input($news_key)."',news_name='".input($news_title)."',last_modified=".time().",news_content='".input($txtdetail)."',news_catalogue='".$categories_id."',keywords='".input($keywords)."',news_shortcontent='".$news_shortcontent."',description='".input($description)."' where news_id = '".$id."'";
			}
			else //insert
			{
				$sql = "insert into news(news_key,news_name,date_added,news_content,news_catalogue,language,keywords,news_shortcontent,description) values ('".input($news_key)."','".input($news_title)."',".time().",'".input($txtdetail)."','".$categories_id."','".$_SESSION['lag']."','".input($keywords)."','".$news_shortcontent."','".input($description)."')";
			}
			$mysql->setQuery($sql);
			if($mysql -> query())
			{
	...
	'error'						=> $error,
	'cat_list'					=> cat_news_list($categories_id),
	'imagesrc'					=> $imagesrc,
	'ImgThumb'					=> $ImgThumb,
	'ImgDel'					=> $ImgDel,
	'keywords'					=> $keywords,
	'description'				=> $description,
	'news_shortcontent'			=>	$news_shortcontent,
	'panel_add_update_product'	=> $panel,
	'news_title'				=> 'Tên đề tin',
	'image'						=> $arr_lang['image'],
	'news_detail'				=> 'Mô tả chi tiết',
