<?php
	...
	{		
		$sql_se = "SELECT image_path FROM image_product WHERE product_id = -1";	

		$mysql -> setQuery($sql_se);	

		$arrImg = $mysql->loadAllRow();		

		$sql_up = "DELETE from image_product WHERE  product_id = -1";	
		...
		if($mysql->query()){
			...
			{	
				...
				echo '../../upload/product_detail/'.$arrImg[$k]['image_path'];	
			}	
		}	
	}
	...
	{	
		...
		{		
			...
			$sql = "SELECT products_id
						  ...
						  ,description,products_image
						  ,products_price,product_encourage
						  ,price_unit,categories_id
						  ,products_description
						  ,product_detail,p_color
						  ,p_attribute,p_type
						  ,p_unit,manufacturer
						  ,species,origin
						  ,manufacturer_link,status
			...
			if(!empty($row['products_image']) )
			{
				...
				{
					$imagesrc = '<a href="'._UPLOAD_IMG_THUMB.$row['products_image'].'"> 
								 <h3>Click here download</h3></a>';
		...
		if(isset($_POST['btnadd']))
		{
			...
			if(!isset($categories_id{0})) {$error= $arr_lang['err_select_cat'];}
	
			else if(!isset($products_name{0})) $error= $arr_lang['err_product_name_short'];
			...
			if(empty($error))
			{
				...
				{					
					//CHINH SUA THEM status tai cho nay
					$sql = "INSERT INTO products(keywords,description
												,products_price,price_unit
												,products_name,products_date_added
												,products_description
												,product_detail,categories_id
												,language,p_color
												,p_type,p_unit
												,product_encourage,manufacturer
												,p_attribute,species
										...
										VALUES ('".$keywords."','".$description."','"
												  .$price."','".$unit."','"
												  .input($products_name)."',".time().",'"
												  .input($products_description)."','"
												  .input($detail)."','".$categories_id."','"
												  .$_SESSION['lag']."','".$p_color."','"
												  .$p_type."','".$p_unit."','"
												  .$encourage."','".$nsx."','"
												  .$p_attribute."','".$checkChoMeo."','"
												  .$Xuatxu."','".$txtlinknsx."','"
												  .$commodity_name."','".input($product_detail_tacdung)."','"
												  .input($product_detail_phuhopcho)."','".input($product_detail_nguyenlieu_thanhphan)."','"
												  .input($product_detail_phantichdambao)."','".input($product_detail_huongdansudung)."','"
												  .input($product_detail_huongdanbaoquan)."','".input($product_detail_luuy)."','"
												  .input($product_detail_khuyenkhich)."','".input($product_detail_donggoi_thetich)."','"
												  .input($product_detail_nhasanxuat)."','".input($product_detail_xuatxu)."')";											  
				...
				{	
					...
					{	
						...
						move_uploaded_file($_FILES ["image"]["tmp_name"],_UPLOAD_IMG_THUMB.$image);
	
						@imagejpeg(resize(_UPLOAD_IMG_THUMB.$image,$ImgW),_UPLOAD_IMG.$image);
	
						$sql = "UPDATE products SET products_image = '".$image."' WHERE products_id = '".$lastId."'";
					...
					$sql = "UPDATE products SET products_key  = '".input($products_key.'-'.$lastId)
					...
					$sql_up_img = "UPDATE image_product SET product_id = ".$lastId." WHERE product_id = -1";
