...
define('MSG_SEND_FORGOT_PASS', 'Một email xác nhận việc lấy lại mật khẩu đã gửi, hãy check mail của bạn.(Một số trường hợp email nằm trong spam.)');
define('MSG_SEND_FORGOT_PASS_SUCCESS', 'Mật khẩu mới đã được gủi tới email của bạn.(Một số trường hợp email nằm trong spam.)');
...
if($_COOKIE["UsNanapetCookie"]!="" and !isset($_SESSION['username'])){
	...
	if(GetOnef('email','user','email="'.$Userdecoded.'" and status >-1')){
		...
		$usr_info = GetOneRow('name','user', 'email = "'.$_SESSION['username'].'"'); //User::getUserInfo($_SESSION['username']);
		//print_r($usr_info);die();
		$_SESSION['name'] = $usr_info['name'];
		//User::getUserInfo($username);
		///kiểm tra tình trạng user
		//$getSatus = GetOneRow('status','user','username="'.$Userdecoded.'"');
		//if($getSatus['status']>-1) // nếu đang hoạt động
		//{
			//update('user',array('last_login'=>time()),'username="'.$Userdecoded.'"');
			//$_SESSION['username'] = $Userdecoded;
		//}else{
