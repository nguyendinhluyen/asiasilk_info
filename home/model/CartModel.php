...
class CartModel{
	...
	public static function saveOrder($cart,$userInfo){
		...
		if(count($cart)>0)
		{
			...
			$sql .= "(username,custommer_name,custommer_email,custommer_addtress,custommer_phone,custommer_fax,custommer_comment,status,order_date,total,custommer_name_receiver,custommer_address_receiver,custommer_phone_receiver,fee_transport,type_of_cash,total_product_transport)"; 
			//fee_transport,transport_district,type_of_transport,type_of_cash,total_product_transport)";
			$sql .= " values ('";
	...
	//================================================================
	//Khiem
	//tao body cho mail xac nhan tu khach hang
	...
	{		
		...
		// lay css cho cac the trong bang
		...
		$score = floor( $total_price / $PriceOfUnit[0]['PriceScore']);			
		...
		$body = $temp->assign_vars($body,array(
					...
					'score'				=> $score,
					'discount'			=> common::convertIntToFormatMoney($discount),					
					// VI TRI SUA					
					'cost'				=> $cost,
					'linkS'				=> $server_name,
				));
