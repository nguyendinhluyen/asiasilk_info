<?php 
	...
	$user_infomation = $xtemplate->load('user_infomation');
	...
	if($_GET['flg'] == 1){// danh sach ua thich
		...
		if(isset($_POST['them'])){// them vao gio hang
			$Cart = new Cart();
			$id_fav = input($_POST['check_fav']);
			foreach ($id_fav as $val){
				$quantity =  $_POST['quantity'.$val];//echo ($quantity);
				$fav_info = GetOneRow('product_key,product_type,product_color,product_price', 'tbl_favorite', ' id = '.$val);
				$Cart->addProduct($fav_info['product_key'], $fav_info['product_color'],$fav_info['product_type'],$fav_info['product_price'],$quantity, 0);//0 add; 1 edit
		...
		$tpl_favorite = $xtemplate->load('favorite_product');
		...
		$tilte_page ='Sản phẩm ưa thích | '.'ASIASILK';
	...
	if($_GET['flg'] == 2){// lich su mua hang
		...
		foreach ($orders as $value) {
			$edit_html = '';
			if($value['status']==0){
				$edit_html = '<div class="edit"><a href="'.$linkS.'gio-hang.html/edit/'.$value['id'].'">Sửa</a> | <a href="'.$linkS.'danh-sach-don-hang.d/'.$value['id'].'" onclick="return xoa();">Xóa</a></div>';
			}
		...
		$tilte_page ='Đơn hàng của tôi | '.'ASIASILK';
	...
	if($_GET['flg'] == 3){//danh sach pet
		...
		$breadcrumbs_path .= '<a href="{linkS}">ASIASILK</a>';
		$breadcrumbs_path .= ' &raquo; '.'Pet profile';
		$tilte_page ='Pet profile | '.'ASIASILK';
	...
	if($_GET['flg'] == 4){// so dia chi
		...
		if(isset($_POST['save_main'])){
			...
			$dataUP = array('address' => $address, 'phone'	=> $phone,'name' => $name);
			update('user', $dataUP, 'email = "'.$_SESSION['username'].'"');
		}
		...
		if(isset($_GET['up']))
		{
			...
			// Vị trí chỉnh sữa cho cập nhật dữ liệu
			if(isset($_POST['save']))
			{								
				...
				if($number_address == '' || $number_street == '' || $number_ward == '' || $number_city == 'Chưa chọn tỉnh/thành' || $number_district == 'Chưa chọn quận/huyện')
				{
					...
					$contact_ed = $xtemplate->load('contact_ed');
																																																	
					list($text_address, $text_street, $text_ward, $text_chung_cu, $text_lau, $text_can_ho,$text_district, $text_city ) = split(' nanapet.com ', $infAdd['street']);
				...
				{					
					...
					$street =  trim($number_address). ' nanapet.com Đường '.trim($number_street).' nanapet.com Phường '.trim($number_ward).'  nanapet.com Chung cư/Tòa nhà '.trim($buildings).' nanapet.com Lầu '.trim($floor). ' nanapet.com Căn hộ '.trim($room).' nanapet.com '.$number_district.' nanapet.com '.$number_city;
			...
			$user = $_SESSION['username'];
			$contact_list = $xtemplate->load('contact_list');
			$listAdd1 = '';
			$listAdd2 = '';
			...
			for($i = 0;$i<$n ;$i++)
			{
				...
				if($i<$ndiv2)
				{															
					$tpl1 = '<table border="0" style="width:260px;"> <tr>
							...
							<tr>
								<td align="center"><a href="'.$linkS.'sua-dia-chi.chm/'.$listAdd[$i]['id'].'">Chỉnh sửa</a> | <a onclick="return deleteContact();" href="xoa-dia-chi-'.$listAdd[$i]['id'].'.chm">Xóa</a></td>
							</tr> </table>';
					$listAdd1 .= $tpl1;
				}
				else{
					$tpl2 = '<table border="0" style="width:260px;"> <tr>
							<td><b>Địa chỉ '.($i+1).'</b></td>
							</tr>
							<tr>
								<td>'.$listAdd[$i]['street'].'</td>
		...
		$tilte_page ='Sổ địa chỉ | '.'ASIASILK';
	}
	
	if($_GET['flg'] == 5){// san pham toi danh gia
		$tieude = "Sản phẩm tôi đánh giá";
		$tplComment = $xtemplate->load('loadCommentUsr');
		$arrComment = GetRows('u.id_product,u.comment,u.date_comment,u.user,r.rate','user_comment u, rate_product r', 'u.user = "'.$_SESSION['username'].'" and u.id_product = r.product_id and u.user = r.user and r.rate > 0 group by u.id_product order by u.date_comment desc');
		...
		if(!empty($arrComment)){
			...
			for($i=0;$i<$n;$i++){
				...
				$product = GetOneRow('products_name','products','products_id = '.$arrComment[$i]['id_product']);
				$name_date = $listrate.'<b> cho sản phẩm '.$product['products_name'].' (ngày '.date("d/m/Y",$arrComment[$i]['date_comment']).') </b>';
		...
		$tilte_page ='Sản phẩm tôi đánh giá | '.'ASIASILK';
	}
}
else
{
