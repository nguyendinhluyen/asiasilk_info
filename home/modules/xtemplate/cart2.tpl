...
<tbody><tr>
	<th class="stt_cart_header">
    	<!--<input type="checkbox" name="check_all" id="check_all" onclick="checkAll();" />-->
    </th>
	<th class="che-bas-cart-heading-image">&nbsp;</th>
	<th class="che-bas-cart-heading-description">Sản phẩm</th>
	<!-- <th class="che-bas-cart-heading-giftwrap">Gift Box</th> -->
 ...
 <!--BEGIN_LISTCART-->
			<tr>
				...
				<td class="che-bas-item-image-cell">
					...
					<a tabindex="-1" href="{linkS}{category}/{product_key}.htm" onclick=""><img class="item-img" src="{linkS}upload/product/{product_image}"  alt="METALLIC ROUCHED LACE DRESS"></a>
				</td>
				...
				<!-- 
				<td class="che-bas-giftwrap-cell che-bas-item-cell">
					<div id="giftBox_0_0" class="che-bas-giftwrap-div">
						<input type="checkbox" checked="true" name="giftwrap_0_0">
						
						<label for="gift-wrap">Gift Box</label>
						<!-- <span><a class="che-add-help-link widget-giftbox-link" href="" onclick="">What's This?</a></span> -->
					<!-- </div>
					<div id="noGiftBox_0_0" class="che-bas-giftwrap-div" style="display: none; ">
						<i>Not available for this item</i>
					</div>
				</td>  -->
				...
				<!--	<form action="{linkS}gio-hang/{product_key}/edit" method="post" name="edit_f{stt}" style="display: inline;"> -->
						<input type="text" title="Cập nhật số lượng trong giỏ" class="glo-form" style="width:28px;height:16px"  size="4" maxlength="4" name="quantity[]" value="{quantity}" />
						<input type="hidden" name="product_key[]" value="{product_key}"/> 
						<input type="hidden" name="type[]" value="{type}"/> 
						<input type="hidden" name="color[]" value="{color}"/> 
						<input type="hidden" name="prices[]" value="{price}"/> 
				       <!-- <a href="javascript: submitformDetele('edit_f{stt}')" >Cập nhật</a>  -->
				    <!-- </form>	-->
					<!-- <form action="{linkS}gio-hang/{product_key}/del" method="post" name="delete_f{stt}" style="display: inline;"> -->
				  	<!--  <input type="hidden" name="type" value="{type}"/> 
					<input type="hidden" name="color" value="{color}"/> -->
					<!--	<ul>
							<li><a href="javascript: submitformDetele('delete_f{stt}')" >Xóa</a> </li>
...
<br/>
	<input type="submit" class="recycle_bin_empty" name="deleteItem"  value="DELETE" onclick="return confirmDelete();"/>
	<input class="order"  type="button" value="Tiếp tục mua hàng" onclick="window.location='{linkS}index.htm'"/>
    
    
    
    <!--<input class="order" style="color: red" type="button" value="ĐẶT HÀNG" onclick="window.location='{linkS}ket-thuc-mua-hang'"/>-->

    <input class="order" style="color: red" type="button" value="ĐẶT HÀNG" onclick="cart_finish_items()"/>
   ...
   <!--<a class="nganluong" style="margin-right:15px" target="_blank" href="https://www.nganluong.vn/button_payment.php?receiver=support@nanapet.com&amp;product_name={list_product}&amp;price={total_price}&amp;return_url=http://nanapet.com"><img src="https://www.nganluong.vn/data/images/merchant/button/btn-paynow-125.png" width="100px" border="0" title="Thanh toán trực tuyến AN TOÀN và ĐƯỢC BẢO VỆ, sử dụng thẻ ngân hàng trong và ngoài nước hoặc nhiều hình thức tiện lợi khác.Được bảo hộ &amp; cấp phép bởi NGÂN HÀNG NHÀ NƯỚC, ví điện tử duy nhất được cộng đồng ƯA THÍCH NHẤT 2 năm liên tiếp, Bộ Thông tin Truyền thông trao giải thưởng Sao Khuê."></a>
   <a class="baokim" style="margin-right:15px" href="https://www.baokim.vn/payment/customize_payment/product?business=support@nanapet.com&product_name={list_product}&product_price={total_price}&product_quantity=1&total_amount={total_price}" target="_blank"><img src="https://www.baokim.vn/application/uploads/buttons/btn_safety_payment_3.png" alt="Thanh toán an toàn với Bảo Kim !" width="100px" height="55px" border="1" title="Thanh toán trực tuyến an toàn dùng tài khoản Ngân hàng (VietcomBank, TechcomBank, Đông Á, VietinBank, Quân Đội, VIB, SHB,... và thẻ Quốc tế (Visa, Master Card...) qua Cổng thanh toán trực tuyến BảoKim.vn" ></a>-->
