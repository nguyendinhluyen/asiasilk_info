...

{
	 ...
	 * str_replace('>','&gt;',$string); $string = str_replace('
	 * ','&nbsp;',$string); $string =
	 * str_replace('\t','&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;',$string);
	 */
...

{
	...
	$str = preg_replace ( "/(ÃƒÂ |ÃƒÂ¡|Ã¡ÂºÂ¡|Ã¡ÂºÂ£|ÃƒÂ£|ÃƒÂ¢|Ã¡ÂºÂ§|Ã¡ÂºÂ¥|Ã¡ÂºÂ­|Ã¡ÂºÂ©|Ã¡ÂºÂ«|Ã„Æ’|Ã¡ÂºÂ±|Ã¡ÂºÂ¯|Ã¡ÂºÂ·|Ã¡ÂºÂ³|Ã¡ÂºÂµ)/", 'a', $str );
	
	$str = preg_replace ( "/(ÃƒÂ¨|ÃƒÂ©|Ã¡ÂºÂ¹|Ã¡ÂºÂ»|Ã¡ÂºÂ½|ÃƒÂª|Ã¡Â»ï¿½|Ã¡ÂºÂ¿|Ã¡Â»â€¡|Ã¡Â»Æ’|Ã¡Â»â€¦)/", 'e', $str );
	
	$str = preg_replace ( "/(ÃƒÂ¬|ÃƒÂ­|Ã¡Â»â€¹|Ã¡Â»â€°|Ã„Â©)/", 'i', $str );
	
	$str = preg_replace ( "/(ÃƒÂ²|ÃƒÂ³|Ã¡Â»ï¿½|Ã¡Â»ï¿½|ÃƒÂµ|ÃƒÂ´|Ã¡Â»â€œ|Ã¡Â»â€˜|Ã¡Â»â„¢|Ã¡Â»â€¢|Ã¡Â»â€”|Ã†Â¡|Ã¡Â»ï¿½|Ã¡Â»â€º|Ã¡Â»Â£|Ã¡Â»Å¸|Ã¡Â»Â¡)/", 'o', $str );
	
	$str = preg_replace ( "/(ÃƒÂ¹|ÃƒÂº|Ã¡Â»Â¥|Ã¡Â»Â§|Ã…Â©|Ã†Â°|Ã¡Â»Â«|Ã¡Â»Â©|Ã¡Â»Â±|Ã¡Â»Â­|Ã¡Â»Â¯)/", 'u', $str );
	
	$str = preg_replace ( "/(Ã¡Â»Â³|ÃƒÂ½|Ã¡Â»Âµ|Ã¡Â»Â·|Ã¡Â»Â¹)/", 'y', $str );
	
	$str = preg_replace ( "/(Ã„â€˜)/", 'd', $str );
	
	$str = preg_replace ( "/(Ãƒâ‚¬|Ãƒï¿½|Ã¡ÂºÂ |Ã¡ÂºÂ¢|ÃƒÆ’|Ãƒâ€š|Ã¡ÂºÂ¦|Ã¡ÂºÂ¤|Ã¡ÂºÂ¬|Ã¡ÂºÂ¨|Ã¡ÂºÂª|Ã„â€š|Ã¡ÂºÂ°|Ã¡ÂºÂ®|Ã¡ÂºÂ¶|Ã¡ÂºÂ²|Ã¡ÂºÂ´)/", 'A', $str );
	
	$str = preg_replace ( "/(ÃƒË†|Ãƒâ€°|Ã¡ÂºÂ¸|Ã¡ÂºÂº|Ã¡ÂºÂ¼|ÃƒÅ |Ã¡Â»â‚¬|Ã¡ÂºÂ¾|Ã¡Â»â€ |Ã¡Â»â€š|Ã¡Â»â€ž)/", 'E', $str );
	
	$str = preg_replace ( "/(ÃƒÅ’|Ãƒï¿½|Ã¡Â»Å |Ã¡Â»Ë†|Ã„Â¨)/", 'I', $str );
	
	$str = preg_replace ( "/(Ãƒâ€™|Ãƒâ€œ|Ã¡Â»Å’|Ã¡Â»Å½|Ãƒâ€¢|Ãƒâ€�|Ã¡Â»â€™|Ã¡Â»ï¿½|Ã¡Â»Ëœ|Ã¡Â»â€�|Ã¡Â»â€“|Ã†Â |Ã¡Â»Å“|Ã¡Â»Å¡|Ã¡Â»Â¢|Ã¡Â»Å¾|Ã¡Â»Â )/", 'O', $str );
	
	$str = preg_replace ( "/(Ãƒâ„¢|ÃƒÅ¡|Ã¡Â»Â¤|Ã¡Â»Â¦|Ã…Â¨|Ã†Â¯|Ã¡Â»Âª|Ã¡Â»Â¨|Ã¡Â»Â°|Ã¡Â»Â¬|Ã¡Â»Â®)/", 'U', $str );
	
	$str = preg_replace ( "/(Ã¡Â»Â²|Ãƒï¿½|Ã¡Â»Â´|Ã¡Â»Â¶|Ã¡Â»Â¸)/", 'Y', $str );
	
	$str = preg_replace ( "/(Ã„ï¿½)/", 'D', $str );
...
// lÃ¡ÂºÂ¥y 1 fill dÃƒÂ²ng dÃ¡Â»Â¯ liÃ¡Â»â€¡u
...
// KiÃ¡Â»Æ’m tra sÃ¡Â»Â± tÃ¡Â»â€œn tÃ¡ÂºÂ¡i
...
// cÃ¡ÂºÂ­p nhÃ¡ÂºÂ­t dÃ¡Â»Â¯ liÃ¡Â»â€¡u
...
// KiÃ¡Â»Æ’m tra phÃ¡ÂºÂ§n mÃ¡Â»Å¸ rÃ¡Â»â„¢ng
...
function pagination($self, $numofpages, $page_num) {
	if ($numofpages > '1') {
		...
		if (($page_num > ($range - $range_min)) && ($numofpages > $range)) {
			$page_pagination .= '<a title="Ban đầu" href="' . $self . 'page=1">Ban đầu</a> ';
		}
		
		if ($page_num != 1) {
			$page_pagination .= '<a href="' . $self . 'page=' . ($page_num - 1) . '">Trang trước</a> ';
		}
		...
		if ($page_num < $numofpages) {
			$page_pagination .= ' <a href="' . $self . 'page=' . ($page_num + 1) . '">Tiếp theo</a>';
		}
		
		if (($page_num < ($numofpages - $range_max)) && ($numofpages > $range)) {
			$page_pagination .= ' <a title="Kết thúc" href="' . $self . 'page=' . $numofpages . '">Kết thúc</a> ';
		}
...
// ========================================================
// Khiem
// tao link cho viec phan trang trong phuong thuc tim kiem moi
...

{
	...
	$arr = split ( '[\?]', $url ); // print_r($arr);die;
	
	$arr = split ( '/order', $arr [0] );
	
	$arr = split ( '/page', $arr [0] ); // print_r($arr);die;
	...
	$end_url = $start . $self; // echo $end_url;die;
